...
public class TestHierarchicalConfigurationEvents extends AbstractTestConfigurationEvents {
     ...
     * In this case, no events should be generated.
     */
    @Test
    public void testAddNodesEmptyEvent() {
        // Arrange
        final BaseHierarchicalConfiguration baseConfig = (BaseHierarchicalConfiguration) config;
        final Collection<? extends ImmutableNode> emptyNodes = new ArrayList<>();

        // Act
        baseConfig.addNodes(TEST_PROPNAME, emptyNodes);

        // Assert
        listener.done(); // Ensures all events are processed (there should be none)

        // Verify no events were received
        assertThrows(AssertionError.class, () -> listener.nextEvent(null),
                "No events should have been recorded when adding an empty list of nodes.");
    }
    ...
    public void testSubConfigurationChangedEventNotConnected() {
        ...
        // Assert
        assertThrows(AssertionError.class, () -> listener.nextEvent(null),
                "No events should be recorded for a disconnected sub-configuration.");
        listener.done(); // Ensures no unexpected events remain
    }
}
